...
async function SaveScenarioAirData(devId, senId, sensorValue, scenario) {
    ...
    try {
        ...
        const sqlInsert = `
            ...
            VALUES (?, ?, ?, ?, ?)
        `;
        await connection.execute(sqlInsert, [devId, senId, sensorValue, currentDateTime, scenarioValue]);

        console.log(`Scenario air data saved for DEV_ID: ${devId}, SEN_ID: ${senId}`);
    } catch (error) {
...
async function saveAirDataToDatabase(devId, hour, dayOfWeek, event) {
    ...
    const sensorNames = [
        'PM10', 'PM25', 'SO2', 'NO2', 'O3', 'CO', 'VOCs', 'H2S', 
        'NH3', 'OU', 'HCHO', 'TEMP', 'HUMI', 'WINsp', 'WINdir', 'BATT', 'FIRM', 'SEND'
    ];
...
// 실행 예시: fishing_boat 시나리오로 10분간 5개 디바이스 데이터 생성
startDataGeneration(9, 3, 'fishing_boat');
