...
function checkTwoVesselsCollision(vesselId1, vesselId2, safeDistance) {
    ...
    const distance = calculateDistance(vessel1.lati, vessel1.longi, vessel2.lati, vessel2.longi);
...
function generateCurvedVesselData(devId, polygon) {
    ...
    let newLongi = state.longi + speedFactor * Math.sin((state.course * Math.PI) / 180) + (curveFactor * 0.00001);
    ...
    return {
        type: 'vessel',
        id: devId,
        log_datetime: formatDateToYYYYMMDDHHMMSS(currentDate), // 제대로 생성된 현재 시간
        rcv_datetime: formatDateToYYYYMMDDHHMMSS(currentDate), // 제대로 생성된 현재 시간
...
function saveVesselDataToDB(vesselData) {
    ...
    sensors.forEach(sensor => {
        ...
        connection.query(sql, values, (err, result) => {
            ...
            console.log(`선박 ${vesselData.id}의 ${sensor.sen_name} 데이터 저장 완료.`);
        });
    });
}
    ...
    const interval = setInterval(() => {
        ...
        for (let i = 1; i <= 100; i++) {
            ...
            if (i === collisionVessels[0] || i === collisionVessels[1]) {
                if (checkTwoVesselsCollision(collisionVessels[0], collisionVessels[1], 0.001)) {
                    console.log(`선박 ${collisionVessels[0]}와 선박 ${collisionVessels[1]}가 충돌했습니다!`);
                }
            }
